...
**[M-2] `app.py` — deprecated Streamlit API**
...
| 파일/폴더 | 상태 |
|---|---|
| `yolo26n.pt` (5.5MB) | 코드 어디에도 참조되지 않음 |
| `data/raw/` | 빈 폴더 — README와 불일치 |
| `data/processed/` | 빈 폴더 — README와 불일치 |
...
| 순위 | 항목 | 기대 효과 |
|---|---|---|
| 1 | `data.yaml` 절대 경로 → 상대 경로 수정 | 이식성 확보 |
| 2 | `augment.py` 플립 라벨 버그 수정 | 학습 데이터 오염 방지 |
| 3 | `app.py` `img_bgr is None` 방어 코드 추가 | 앱 크래시 방지 |
| 4 | `use_column_width` → `use_container_width` 업데이트 | 경고 제거 |
| 5 | YOLO11s/m 모델로 재학습 | Recall 0.65+ 목표 |
| 6 | `analysis.ipynb` Section 4 실제 추론 결과로 교체 | 포트폴리오 신뢰도 향상 |
| 7 | README에 데모 GIF / 스크린샷 추가 | 첫인상 개선 |
| 8 | ONNX Export 추가 (`model.export(format='onnx')`) | 배포 가능성 시연 |
